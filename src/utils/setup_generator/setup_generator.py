...
class Constants:
    ...
    KEY_BASE_DOCKERFILE_TEMPLATE = "dockerfile_template"
    ...
    def __get_infos(self, data, config_filename, init_section=True, property_prefix=None, env_var_prefix=None):
        ...
        for property, value in data.items():
            ...
            if isinstance(value, dict):
                load_fn_calls.extend(self.__get_infos(value, config_filename, init_section, property_name, env_var_name))
            else:
                if value.find("=") == 0:  # standard load config statement
                    ...
                    load_fn_calls.append(Constants.CONFIG_LOADER_OPT_SUBST_STATEMENT_FMT.format(
                        ...
                        check=tokens[0][1:].strip(),
                        substitution_not_null=tokens[1].strip(),
                        substitution_null=tokens[2].strip(),
                        config_filename=config_filename
                    ))
                else:
...
if __name__ == "__main__":
    SetupGenerator(sys.argv[1]).generate()
